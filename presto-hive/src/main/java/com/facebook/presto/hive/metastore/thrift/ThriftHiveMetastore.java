        ...
        implements HiveMetastore
{
    ...
    public Map<String, PartitionStatistics> getPartitionStatistics(String databaseName, String tableName, Set<String> partitionNames)
    {
        ...
        Map<String, HiveBasicStatistics> partitionBasicStatistics = getPartitionsByNames(databaseName, tableName, ImmutableList.copyOf(partitionNames))
                .stream()
                .collect(toImmutableMap(partition -> makePartName(partitionColumns, partition.getValues()), partition -> getHiveBasicStatistics(partition.getParameters())));
        ...
        for (String partitionName : partitionNames) {
            ...
            if (basicStatistics == null) {
                basicStatistics = createEmptyStatistics();
            }
            Map<String, HiveColumnStatistics> columnStatistics = partitionColumnStatistics.get(partitionName);
    ...
    public synchronized void updateTableStatistics(String databaseName, String tableName, Function<PartitionStatistics, PartitionStatistics> update)
    {
        ...
        Table metastoreApiTable = getTable(databaseName, tableName)
                .orElseThrow(() -> new TableNotFoundException(new SchemaTableName(databaseName, tableName)));
        metastoreApiTable.setParameters(updateStatisticParameters(metastoreApiTable.getParameters(), updatedStatistics.getBasicStatistics()));
        alterTable(databaseName, tableName, metastoreApiTable);

        com.facebook.presto.hive.metastore.Table table = fromMetastoreApiTable(metastoreApiTable);
    ...
    {
        PartitionStatistics currentStatistics = requireNonNull(getPartitionStatistics(databaseName, tableName, ImmutableSet.of(partitionName)).get(partitionName), "currentStatistics is null");
        PartitionStatistics updatedStatistics = update.apply(currentStatistics);

        List<Partition> partitions = getPartitionsByNames(databaseName, tableName, ImmutableList.of(partitionName));
        verify(partitions.size() == 1, "exactly one partition is expected to be returned: %s", partitions.size());

        Partition partition = getOnlyElement(partitions);
        partition.setParameters(updateStatisticParameters(partition.getParameters(), updatedStatistics.getBasicStatistics()));
