        ...
        extends SqlOperator
{
    ...
    private static Class<?> generateIndeterminate(Type type, FunctionRegistry functionRegistry)
    {
        ...
        byte[] md5Suffix = Hashing.goodFastHash(128).hashBytes(type.toString().getBytes(UTF_8)).asBytes();

        ClassDefinition definition = new ClassDefinition(
                a(PUBLIC, FINAL),
                makeClassName(Joiner.on("$").join("RowIndeterminateOperator", BaseEncoding.base16().encode(md5Suffix))),
                type(Object.class));
