...
public class SymbolMapper
{
    ...
    private PartitioningScheme canonicalizePartitionFunctionBinding(PartitioningScheme scheme, PlanNode source)
    {
        ...
        for (Symbol symbol : source.getOutputSymbols()) {
            Symbol canonicalOutput = map(symbol);
    ...
    {
        Map<Symbol, Aggregation> aggregations = statisticAggregations.getAggregations()
                .entrySet()
                .stream()
                .collect(toImmutableMap(entry -> map(entry.getKey()), entry -> map(entry.getValue())));
        List<List<Symbol>> groupingSets = statisticAggregations.getGroupingSets()
                .stream()
                .map(this::map)
                .collect(toImmutableList());
        return new StatisticAggregations(aggregations, groupingSets);
    }
